...
big_square_size = 2
width = int(big_square_size // square_size)
number_of_cells = width ** 2
map.data = [0 for i in range(number_of_cells)]
start_map = map.data.copy()
map.info.width = width
map.info.height = width
x_start = -(big_square_size / 2) #map.info.origin.position.x
y_start = -(big_square_size / 2) #map.info.origin.position.y
map.info.origin.position.x = x_start
map.info.origin.position.y = y_start
